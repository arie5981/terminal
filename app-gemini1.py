...
# ה-SDK החדש של גוגל מנהל בעצמו תזמוני Timeout ורובוסטיות בצורה אופטימלית
...
def chat():
    ...
    # ה-SDK של גוגל מצפה למבנה של מודל (model) ומשתמש (user)
    formatted_contents = []
    ...
    for msg in chat_history:
        ...
        formatted_contents.append(
            types.Content(
                role=gemini_role,
                parts=[types.Part.from_text(text=clean_html_for_history(msg.get("content", "")))]
            )
        )
    ...
    try:
        # פנייה למודל ג'מיני באמצעות ה-SDK הרשמי והעדכני
        ...
        return jsonify({"response": "מצטער, נתקלתי בשגיאה בתקשורת עם שרת ה-AI של גוגל. אנא נסה שוב."})
