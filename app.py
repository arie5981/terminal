...
def get_or_create_context_cache(client):
    ...
    # במידה וכבר קיים מזהה קאש בזיכרון, נבצע בדיקת תקינות מול גוגל לראות שהוא עדיין תקף
    if CACHE_NAME:
        try:
    ...
    # בניית ה-System Instruction המלא כולל ה-Context (יוזרק רק לתוך ה-Cache פעם אחת)
    system_instruction = (
