...
def clean_html_for_history(text):
    ...
    if not text:
        return ""
    # הסרת בלוק המטא-דאטה שנמצא אחרי המפריד
    if "<br><hr>" in text:
        text = text.split("<br><hr>")[0]
    # הסרת תגיות ה-HTML שנוצרו מה-Markdown
    clean = re.compile('<.*?>')
...
def chat():
    ...
    if clean_history:
        try:
            ...
            # הוספת 2 ההודעות האחרונות מההיסטוריה בלבד לצורך קבלת הקשר קרוב
    ...
    # --- 4. הבניית הודעת המערכת (System Prompt) המהודקת והקשוחה ---
    messages = [
        {
            "role": "system", 
            "content": (
                ...
                "2. אסור להמציא פעולות, כותרות או שלבי ביניים שאינם כתובים בטקסט המקור רק כדי לייצר מבנה מורכב יותר.\n"
                "3. אם המשתמש שואל אם משהו אפשרי, וקיימת בנהלים דרך מסוימת לבצע זאת, פתח מיד ובאופן חיובי בהנחיות המעשיות לביצוע (לדוגמה: 'כדי לקבל את הקוד חד פעמי לאימייל, יש לבצע את הפעולות הבאות:').\n\n"
                ...
                "5. דגשים מיוחדים: הערות חשובות או אזהרות המופיעות במקור יוצגו בשורה נפרדת וממודגשת (למשל: '**שים לב:**' או '**שלב קריטי:**').\n\n"
    ...
    # הזרקת ההיסטוריה הנקייה ללולאת השיחה של המודל הסופי
    for msg in clean_history:
        messages.append({"role": msg["role"], "content": msg["content"]})

    # שליחת השאלה הנוכחית יחד עם הקונטקסט שמוצה מהשאילתה המשודרגת
    messages.append({
        "role": "user",
    ...
    try:
        ...
        # magnetism של הקישורים על גבי ה-HTML הסופי
        final_answer = inject_hyperlinks(html_answer)
        
        # חיבור המטא-דאטה החדש והמוסתר ישירות לתוך ה-HTML שחוזר למשתמש
