...
def chat():
    ...
    # הגדלנו ל-6 כדי להבטיח ששני סוגי ייפוי הכוח ייכנסו ל-Context יחד (הקוד המקורי והעובד שלך)
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "2. אם המשתמש שואל אם משהו אפשרי, וקיימת בנהלים דרך מסוימת לבצע זאת, פתח מיד ובאופן חיובי בהנחיות המעשיות לביצוע (לדוגמה: 'כדי לקבל את הקוד החד פעמי לאיมייל, יש לבצע את הפעולות הבאות:').\n\n"
                ...
                "3. סיום נקי: אל תוסיף משפטי סיום רובוטיים קבועים (כמו 'אני כאן לכל שאלה, אל תהסס פנות'). אם יש צורך, חתום במשפט קצר וטבעי, או סיים ישירות בסיכום הנתונים.\n\n"
                "🧱 חוקי מבנה וארכיטקטורה דינמית (חובה לכל שאלה):\n"
                "1. אבחנה בין שלבים לאפשרויות: \n"
                "   - אם הטקסט מתאר מספר דרכים חלופיות לבחירה, השתמש בכותרות כגון: '**אפשרות 1: [שם האפשרות]**', '**אפשרות 2: [שם האפשרות]**'.\n"
                "   - השתמש במילה '**שלב**' (למשל: '**שלב 1: [שם השלב]**') אך ורק אם מדובר בתהליך כרונולוגי חובה שבו שלב ב' תלוי בביצוע שלב א'.\n"
                ...
                "5. דגשים מיוחדים: הערותחשובות או אזהרות יוצגו בשורה נפרדת ומודגשת (למשל: '**שים לב:**' או '**שלב קריטי:**').\n\n"
                "🔗 חוקי קישורים וסוגריים מרובעים (קריטי להפעלת הקישורים):\n"
                "- עליך לוודא שכל שם של טופס, אתר, מערכת או כתובת מייל שמופיעים בטקסט, עטופים בדיוק בסוגריים מרובעים כפי שהם מופיעים בנהלים (לדוגמה: [אתר שירות אישי], [addmy@nioi.gov.il]). אל תשמיט ואל שנה את הסוגריים המרובעים האלו בתוך חלקי המידע."
            )
        }
    ]
